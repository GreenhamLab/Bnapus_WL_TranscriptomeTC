...
There are a number of files/objects that we provide on our Github page (https://github.com/GreenhamLab/Bnapus_WL_TranscriptomeTC) that will need to be downloaded before starting. 

You will need to download the following files and save them in the 'R_input' folder you just created:
...
These R objects were saved collectively as "RData_BnapusWL.RData". When you load this object you should see these files populate:
...
```{r Figure 7}
gsls = read_xlsx(file.path(inPath, file = "MetabolomicsData.xlsx"), sheet = "R_output_DblExclude")
## Clean up the DF
gsls = gsls[, -c(5:6)]

## Make separate Sulfoquinose and RA columns (non-GSLs)
NonGSL = gsls %>% select(!c(SIN:NAS))

gsls = gsls[,-c(13:15)]

## Pivot the GSLs into a single column to plot
gsls = gsls %>% pivot_longer(cols = 5:12, names_to = "GSL", values_to = "PeakArea")

gsls = gsls %>% group_by(GSL,Genotype, Treatment, ZT, BioRep) %>% 
  mutate(BioAvg_GSL = mean(PeakArea)) %>% ungroup()

## Log transform the peak areas
NonGSL$LogArea_RA = log(NonGSL$RA)
NonGSL$LogArea_Sulfo = log(NonGSL$SQ)
NonGSL$LogArea_Sucrose = log(NonGSL$Sucrose)

gsls$LogArea_GSL = log(gsls$PeakArea)

## Add an average column that averages over the tech reps to create one point per biorep
NonGSL = NonGSL %>% group_by(Genotype, Treatment, ZT, BioRep) %>% 
  mutate(BioAvg_RA = mean(RA), 
         BioAvg_Sulfo = mean(SQ)) %>% ungroup()

## Add an average column that averages over the tech reps to create one point per biorep
gsls = gsls %>% group_by(GSL,Genotype, Treatment, ZT, BioRep) %>% 
  mutate(BioAvg_GSL = mean(PeakArea), 
         Log_BioAvg_GSL = log(BioAvg_GSL)) %>% ungroup()

NonGSL = NonGSL %>% group_by(Genotype, Treatment, ZT, BioRep) %>% 
  mutate(BioAvg_RA = mean(RA), 
         BioAvg_Sulfo = mean(SQ),
         BioAvg_Sucrose = mean(Sucrose), 
         Log_BioAvg_Sulfo = log(BioAvg_Sulfo), 
         Log_BioAvg_RA = log(BioAvg_RA), 
         Log_BioAvg_Sucrose = log(BioAvg_Sucrose)) %>% ungroup()


# Remove PeakArea and pull unique rows only to keep one point per biorep in the plots
gsls = gsls %>% select(!PeakArea) %>% unique()
  # Remove LogArea GSL to have one datapoint per plant per GSL
gsls = gsls %>% select(!LogArea_GSL) %>% unique()

## Plot all to inspect cool patterns
gsls_List = split(gsls, gsls$GSL)

# GJV and IBE are under our threshold of what we are confident in; skip
Fig7b_List = gsls_List[-c(3:4)]

# We also do not feel confident about NAP, NAS, or PRO, SIN levels for St so we remove these as well
names(Fig7b_List)

# Since the two accessions have different GSLs name them individually and plot
Fig7b_List_St = Fig7b_List[-c(3:6)]
Fig7b_List_St = lapply(Fig7b_List_St, function(x) {x = x %>% filter(Genotype == "St"); x})
Fig7b_List_Mu = lapply(Fig7b_List, function(x) {x = x %>% filter(Genotype == "Mu"); x})

pdf(file.path(FigPath, file = "Figure7b.pdf"))
for(i in 1:length(Fig7b_List_St)){
  print(Fig7b_List_St[[i]] %>% 
          filter(Log_BioAvg_GSL != "-Inf") %>%
          ggplot(aes(x = factor(ZT, levels = c("4", "12", "20")), 
                     y = Log_BioAvg_GSL, color = Treatment, fill = Treatment)) +
          geom_boxplot(alpha = .6, outlier.colour = NA, outlier.fill = NA) +
          geom_point(aes(group = Treatment), position=position_dodge(.73)) +
          ...
          ylab("Peak Area (log)") +
          theme(legend.position = "bottom") +
          ggtitle(paste("St:", names(Fig7b_List_St[i]))))
}
for(i in 1:length(Fig7b_List_Mu)){
  print(Fig7b_List_Mu[[i]] %>% 
          filter(Log_BioAvg_GSL != "-Inf") %>%
          ggplot(aes(x = factor(ZT, levels = c("4", "12", "20")), 
                     y = Log_BioAvg_GSL, color = Treatment, fill = Treatment)) +
          geom_boxplot(alpha = .6, outlier.colour = NA, outlier.fill = NA) +
          ...
          ylab("Peak Area (log)") +
          theme(legend.position = "bottom") +
          ggtitle(paste("Mu:", names(Fig7b_List_Mu[i]))))
}
dev.off()

### Statistics
st_4me_gsls = gsls %>% filter(Genotype == "St" & GSL == "4ME (Sm)" & Log_BioAvg_GSL != "-Inf")
t.test(Log_BioAvg_GSL ~ Treatment, data = st_4me_gsls, subset = st_4me_gsls$ZT == 4)

st_nas_gsls = gsls %>% filter(Genotype == "St" & GSL == "NAS" & Log_BioAvg_GSL != "-Inf")
t.test(Log_BioAvg_GSL ~ Treatment, data = st_nas_gsls, subset = st_nas_gsls$ZT == 4)

mu_4me_gsls = gsls %>% filter(Genotype == "Mu" & GSL == "4ME (Sm)" & Log_BioAvg_GSL != "-Inf")
t.test(Log_BioAvg_GSL ~ Treatment, data = mu_4me_gsls, subset = mu_4me_gsls$ZT == 4)

mu_nas_gsls = gsls %>% filter(Genotype == "Mu" & GSL == "NAS" & Log_BioAvg_GSL != "-Inf")
t.test(Log_BioAvg_GSL ~ Treatment, data = mu_nas_gsls, subset = mu_nas_gsls$ZT == 4)

mu_pro_gsls = gsls %>% filter(Genotype == "Mu" & GSL == "PRO" & Log_BioAvg_GSL != "-Inf")
t.test(Log_BioAvg_GSL ~ Treatment, data = mu_pro_gsls, subset = mu_pro_gsls$ZT == 4)
```
